<template>
  <article v-if="recette" class="recette">
    ...
    <section id="recette__ingredients__etapes" class="md:flex">
      ...
      >
        <h1 class="lg:my-8 my-2 text-2xl font-bold text-yellow-500">
          Ingrédients
        </h1>
      ...
      <section class="recette__etapes md:w-2/3 w-full">
        ...
        <div
          ...
          :key="index"
          class="flex pb-5 mb-6 border-b border-gray-200"
        >
          <div class="flex items-center justify-center w-1/12">
            <div
              class="flex items-center justify-center w-10 h-10 text-sm border-2 border-blue-200 rounded-full"
            >
              <div class="font-bold text-blue-400">{{ index + 1 }}</div>
            </div>
          </div>
          <div class="flex items-center justify-center w-11/12">
            <p class="text-md w-full mr-2 text-gray-600">
              {{ preparation.etape }}
            </p>
            <div
              v-if="preparation.image"
              class="w-28 flex items-center justify-center"
            >
              <img
                :src="
                  $config.strapiURL + preparation.image.formats.thumbnail.url
                "
                :alt="`Etape de préparation ${index + 1}`"
    ...
    <section v-if="recette.notabene" class="notabene">
...
export default {
  ...
  async asyncData(context) {
    const data = await context.$graphql.default.request(RECETTE, {
